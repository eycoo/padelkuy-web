...
document.addEventListener('DOMContentLoaded', () => {
    // 2. LOGIKA AUTH, NAVIGASI, & FETCH VENUE API
    ...
    // Fungsi fetch data Venue asli dari API Backend
    const fetchVenues = async (city = '') => {
        venueGrid.innerHTML = '<p class="empty-state">Memuat venue...</p>';
        
        try {
            // Jika city ada nilainya dan bukan 'all', tambahkan parameter URL
            const url = (city && city !== 'all') ? `/api/venues.php?city=${encodeURIComponent(city)}` : '/api/venues.php';
        ...
        venueGrid.innerHTML = venues.map(v => {
            ...
            <div class="court-card">
                <div class="court-image">
                    <img src="${v.image_path}" alt="${v.name}" onerror="this.src='assets/images/login-bg.png'">
                </div>
                <div class="court-body">
                    <h3>${v.name}</h3>
                    <span class="court-tag">${v.city}</span>
                    <div class="court-foot">
                        <div class="price" style="display: flex; flex-direction: column; line-height: 1.2;">
                            <span style="font-size: 11px; color: var(--muted); font-weight: normal;">Start from</span>
                            <span>${formatPrice} <small>/ hr</small></span>
                        </div>
    ...
    // Trigger pencarian saat tombol diklik atau dropdown diubah
    if (searchBtn && cityFilter) {
        searchBtn.addEventListener('click', () => {
            fetchVenues(cityFilter.value);
        });
    } else if (cityFilter) {
        cityFilter.addEventListener('change', (e) => {
            fetchVenues(e.target.value);
        });
    }
